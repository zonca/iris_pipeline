...
PACKAGENAME = metadata.get('package_name', 'iris_pipeline')
DESCRIPTION = metadata.get('description', 'Data Reduction System (DRS) for the Thirty Meter Telescope IRIS imager/spectrograph')
AUTHOR = metadata.get('author', 'Andrea Zonca, Arun Surya')
AUTHOR_EMAIL = metadata.get('author_email', '')
LICENSE = metadata.get('license', 'unknown')
URL = metadata.get('url', 'https://github.com/oirlab/iris_pipeline')
__minimum_python_version__ = metadata.get("minimum_python_version", "3.6")

# Enforce Python version check - this is the same check as in __init__.py but
# this one has to happen before importing ah_bootstrap.
if sys.version_info < tuple((int(val) for val in __minimum_python_version__.split('.'))):
    sys.stderr.write("ERROR: iris_pipeline requires Python {} or later\n".format(__minimum_python_version__))
    sys.exit(1)
