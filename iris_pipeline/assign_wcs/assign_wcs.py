...
def load_wcs(input_model, reference_files={}):
    ...
    if "wcsinfo" not in input_model.meta:
        ...
        shift_by_crpix = models.Shift(-(input_model.meta.wcsinfo.crpix1 - 1)*u.pix) & models.Shift(-(input_model.meta.wcsinfo.crpix2 - 1)*u.pix)
        pix2sky = getattr(models, "Pix2Sky_{}".format(input_model.meta.wcsinfo.ctype1[-3:]))()
        celestial_rotation =  models.RotateNative2Celestial(
                    input_model.meta.wcsinfo.crval1*u.deg, input_model.meta.wcsinfo.crval2*u.deg, 180*u.deg)
        pix2sky.input_units_equivalencies = {"x": u.pixel_scale(input_model.meta.wcsinfo.cdelt1 *u.deg/u.pix),
        ...
        detector_frame = cf.Frame2D(name="detector", axes_names=("x", "y"),
                                            unit=(u.pix, u.pix))
        sky_frame = cf.CelestialFrame(reference_frame=getattr(coord,input_model.meta.coordinates.reference_frame)(), name='sky_frame', unit=(u.deg, u.deg))
